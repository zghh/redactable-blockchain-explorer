...
export class SyncServices {
	...
	async processBlockEvent(client, block, noDiscovery, deleteTransaction) {
		...
		for (let txIndex = 0; txIndex < txLen; txIndex++) {
			...
			if (
			...
			} else if (txObj.payload.header.channel_header.typeString === 'REVOKE') {
				rwset =
					txObj.payload.data.transaction.payload.data.actions[0].payload.action
						.proposal_response_payload.extension.results.ns_rwset;
				readSet = rwset.map(rw => ({
					chaincode: rw.namespace,
					set: rw.rwset.reads
				...
				redactBlockNum.push(txObj.payload.data.transaction.block_number);
			} else if (
			...
			const transaction_row = {
				...
				redactor: txObj.payload.header.redactor,
		...
		if (redactBlock || redactBlockNum.length > 0) {
			function sleep(ms) {
	 ...
	 * @returns
	 */
	async processRevokeTransactionEvent(client, revokeTransaction) {
		const channel_name =
			revokeTransaction.transaction.payload.header.channel_header.channel_id;
		const block = await client.fabricGateway.queryBlock(
			channel_name,
			revokeTransaction.transaction.block_number
		);
		try {
			if (block) {
				await this.processBlockEvent(client, block, true, false);
			...
			logger.error(
				`Failed to process Block # ${revokeTransaction.transaction.block_number}`
			);
		}
	}
